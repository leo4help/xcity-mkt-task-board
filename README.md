...
| ID | 夥伴名 | 關聯專案 | 關聯任務 | 支援項目/細節 | 狀態 | 期限 | 備注 |
|----|--------|----------|----------|----------------|------|------|------|

- **ID**：自動產生（`S-1`、`S-2`...，故意不用單一字母，避免專案分頁排到字母 P 時跟舊版 ID 撞在一起），**分頁第一欄一定要是「ID」**，不然後端會對錯欄位。如果你是手動建的這個分頁、目前欄位是從「夥伴名」開始（沒有 ID 這欄），在 Apps Script 編輯器執行一次 `fixPartnerSheetHeaders()`，會自動在 A 欄前面插入「ID」標題、既有資料往右移，不用手動調整。
- **夥伴名**：預設列出 Andy／Charlie／Leo／溫／培培／朵朵，也可以直接打新名字新增（跟新增專案一樣，是 datalist 自由輸入，不是固定選單）。
- **關聯專案**（選填）：既有專案分頁的下拉選單。
- **關聯任務**（選填）：選了關聯專案後才會出現，只列出「該專案裡有標記需要支援」的任務，避免清單太長找不到重點。
- **支援項目/細節**：文字輸入，寫清楚要對方支援什麼。
- **狀態**：`To Do` / `Doing` / `Done`（跟專案任務的狀態欄位不同，是獨立的三態）。
- **期限**：預計支援交付日，`yyyy-MM-dd`。
- **備注**：純文字補充。

執行一次 `setupSheet()`（或部署 Code.gs 後任何一次寫入）會自動確保這個分頁存在且標題列正確；如果你已經手動建好但欄位對不齊，比照上面「ID 欄位」的說明手動調整。
