...
- **ID**：自動產生（P1、P2...），**分頁第一欄一定要是「ID」**，不然後端會對錯欄位。如果你是手動建的這個分頁、目前欄位是從「夥伴名」開始（沒有 ID 這欄），在 Apps Script 編輯器執行一次 `fixPartnerSheetHeaders()`，會自動在 A 欄前面插入「ID」標題、既有資料往右移，不用手動調整。
